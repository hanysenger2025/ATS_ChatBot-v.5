...
const App: React.FC = () => {
  ...
  // Storage key for v5
  const FAVORITES_STORAGE_KEY = 'ats_favorites_v5_official';
  ...
  const handleSelectSchool = (schoolName: string) => {
    ...
    resetFilters();
    handleSendMessage(`أريد معلومات كاملة وموثقة من قاعدة بيانات v5 عن مدرسة ${schoolName}`);
  };
    ...
    <div className="flex flex-col h-screen max-h-screen bg-slate-50 overflow-hidden font-['Cairo'] text-slate-900">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">ATS Assistant <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg mr-1 border border-indigo-100">v5 Official</span></h1>
        ...
        <div className="flex gap-2">
          <button onClick={() => setIsSearchOpen(true)} className="p-2.5 bg-white hover:bg-indigo-50 text-indigo-600 rounded-xl transition-all flex items-center gap-2 font-bold shadow-sm border border-slate-200 group">
            <Filter className="w-5 h-5 group-hover:rotate-12 transition-transform" /> <span className="hidden sm:inline text-sm">تصفح مدارس v5</span>
          </button>
          ...
          <div className="flex justify-end animate-in fade-in">
             <div className="flex flex-row-reverse gap-4">
              ...
              <div className="bg-white p-5 rounded-3xl border border-slate-200 flex items-center">
                ...
                <span className="text-sm text-slate-400 font-bold">جاري مراجعة سجلات v5...</span>
              </div>
            </div>
          </div>
        )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 pb-32">
                {filteredSchools.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-32 bg-white rounded-[4rem] border-2 border-dashed border-slate-200">
                    ...
                    <h4 className="text-2xl font-black text-slate-800 mb-3"> لم نجد نتائج في سجلات v5 </h4>
